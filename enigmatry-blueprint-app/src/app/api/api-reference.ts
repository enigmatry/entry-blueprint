...
export interface IProductsClient {
     ...
     * @param code Code
     */
    isCodeUnique(code: string | null): Observable<GetProductCodeUniquenesResponse>;
}
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<GetProductCodeUniquenesResponse>><any>_observableThrow(e);
                }
            } else
                return <Observable<GetProductCodeUniquenesResponse>><any>_observableThrow(response_);
        }));
    }

    protected processIsCodeUnique(response: HttpResponseBase): Observable<GetProductCodeUniquenesResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = GetProductCodeUniquenesResponse.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status === 404) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<GetProductCodeUniquenesResponse>(<any>null);
    }
}
...
export class GetProductCodeUniquenesResponse implements IGetProductCodeUniquenesResponse {
    isUnique?: boolean;

    constructor(data?: IGetProductCodeUniquenesResponse) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): GetProductCodeUniquenesResponse {
        data = typeof data === 'object' ? data : {};
        let result = new GetProductCodeUniquenesResponse();
        result.init(data);
        return result;
    }
...
export interface IGetProductCodeUniquenesResponse {
    isUnique?: boolean;
}
